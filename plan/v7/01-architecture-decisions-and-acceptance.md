...
- `WorkflowAgent` on the Workflow DevKit with `@workflow/world-postgres` owns durable turn execution (ADR 0008, revised; adopted with the pinned realm patch).
...
The Workflow DevKit runtime (WorkflowAgent, Postgres World, hooks, the Nitro compiler host) is adopted per the revised ADR 0008 — a user decision made 2026-07-11 after the compatibility gate ran twice. Reference checkouts remain evidence, not extra adoption license: introducing further upstream subsystems still requires a separate, user-approved decision tied to a concrete product need.
...
5. **Feature cut list:** the canonical inventory is [ADR 0007's feature-disposition table](../../docs/adr/0007-native-ui-stream-tools-and-approval-profile.md#feature-disposition). Every old activity, recovery, and turn-completion feature is explicitly kept, redesigned, or deleted there with its user-visible consequence. Step 08 adds any disposition discovered by executable parity work to that same table instead of creating a second list.
...
ADRs: `docs/adr/0006-ai-sdk-7-native-core.md`, `0007-native-ui-stream-tools-and-approval-profile.md`, and `0008-workflow-durable-execution-substrate.md` (revised 2026-07-11 to the adopted-with-patch outcome).
