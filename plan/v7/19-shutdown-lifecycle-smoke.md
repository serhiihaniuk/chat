...
The process shuts down in a proven order within a bounded budget under every condition (including a blocked provider), repeated signals are safe, and one command proves the whole lifecycle — start, stream, cancel, crash-resume — with no real credentials.
...
2. drain: wait for active turns up to the budget; remaining runs persist/suspend — the worker resumes them after restart (deploys don't kill turns; this is the substrate's operational superpower);
3. close streams/keepalive; close the Hono server;
4. stop the world worker; close DB pools last;
...
4. start a third turn, `kill -9` the process mid-stream, restart, assert the run resumes to terminal and a reconnect stream delivers it;
5. graceful shutdown; assert exit code, drain behavior, zero leaked handles/child processes;
6. rerun the permanent Step 02 compatibility suite against the production-shaped composition, then re-measure rows/turn with real tools and the final coalescing state.

## Edge cases (each a test)

1. shutdown with an idle server → immediate clean exit;
2. shutdown mid-stream → run suspends within the drain budget; post-restart resume proven (smoke item 4);
3. shutdown with a blocked provider (never-resolving mock) → exits within budget + grace, stage timeout recorded;
4. double SIGTERM → single orderly shutdown;
5. boot failure (bad DB URL) → partial resources released, port never opened, nonzero exit;
6. gauges/probes at zero after the full smoke.
