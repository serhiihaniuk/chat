...
A plain-text side call on the configured title model, bounded by `settings.timeouts.titleMs`, is triggered after a completed turn while the conversation remains untitled. The dedicated idempotent `generateConversationTitle` workflow normalizes the first returned line to the existing 2–6 word contract, then conditionally updates only an empty title so replay and races cannot retitle. A failed or timed-out attempt leaves the conversation eligible for a later completed turn to retry; the first successful conditional write wins. A schema-bearing `Output.object` must not cross WorkflowAgent's internal workflow-to-step boundary: schema objects contain constructors and functions that are not durable values. Failure logs safely and never touches turn status.
...
4. title model failure/timeout → turn unaffected, safe log, no title; a later completed turn may retry while the title is still empty;
5. title success → conversation title persisted once; later turns do not retitle (per the verified rule);
...
| Calm cancellation                             | Cancellation is an `error(aborted)` wire terminal even though the durable status is user-aborted.                                        | The new wire uses native `abort`; persistence uses `cancelled`; pre-output cancellation stores no assistant message and mid-stream visible text/reasoning is recovered from the Workflow journal into durable history.                                      | **Intentional improvement:** the widget renders a calm stopped state, and refresh preserves what the user already saw.                                                  |
| Conversation title                            | Only the initial exchange is eligible; the write is conditional; generation failure never changes the completed turn.                    | A dedicated WorkflowAgent job uses normalized plain text, configured model/timeout, untitled-conversation eligibility, and a conditional write; a later completed turn can recover a failed attempt, and run completion does not await title completion.    | **Intentional reliability improvement:** the first successful title still wins, but a transient first-attempt failure no longer leaves the fallback label permanent.    |
...
- `generate-conversation-title.ts` checks the empty-title rule before submitting work, leaving failed attempts eligible for a later completed-turn retry;
