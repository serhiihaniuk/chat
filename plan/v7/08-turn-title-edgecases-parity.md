...
| Conversation title                            | Only the initial exchange is eligible; the write is conditional; generation failure never changes the completed turn.                    | A dedicated WorkflowAgent job uses structured output, configured model/timeout, initial-exchange eligibility, and a conditional write; run completion does not await title completion.                                                                      | **Parity proven.**                                                                                                                                                      |
...
- the production workflow uses a dedicated `WorkflowAgent`, `Output.object`, `models.titleModelId`, and `timeouts.titleMs`, with no tools;
