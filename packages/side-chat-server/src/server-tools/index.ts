...
export type ServerToolInvocation = Readonly<{
...
export type ServerToolExecutionContext = Readonly<{
  /** Secret-free actor identity reconstructed from the authenticated request. */
  actor: DurableActorRef;
  /** Durable invocation identity available to authorization and audit adapters. */
  invocation: ServerToolInvocation;
  /** Stable identity that a mutating adapter must use for idempotency. */
  executionKey: string;
  /** Available only in execution realms that configure nested model generation. */
