...
const HANDLES: readonly { handle: ResizeHandle; ariaLabel: string; className: string }[] = [
  {
    ...
    className:
      "absolute top-0 left-0 z-(--layer-panel-resize-handle) size-4 cursor-nwse-resize bg-transparent focus:outline-none max-sm:hidden",
  },
  {
    handle: "top-right",
    ariaLabel: "Resize panel from top right",
    className:
      "absolute top-0 right-0 z-(--layer-panel-resize-handle) size-4 cursor-nesw-resize bg-transparent focus:outline-none max-sm:hidden",
  },
  {
    handle: "left",
    ariaLabel: "Resize panel from left edge",
    className:
      "absolute top-4 bottom-4 left-0 z-(--layer-panel-resize-handle) w-2 cursor-ew-resize bg-transparent focus:outline-none max-sm:hidden",
  },
  {
    handle: "right",
    ariaLabel: "Resize panel from right edge",
    className:
      "absolute top-4 right-0 bottom-4 z-(--layer-panel-resize-handle) w-2 cursor-ew-resize bg-transparent focus:outline-none max-sm:hidden",
  },
  {
    handle: "top",
    ariaLabel: "Resize panel from top edge",
    className:
      "absolute top-0 right-4 left-4 z-(--layer-panel-resize-handle) h-2 cursor-ns-resize bg-transparent focus:outline-none max-sm:hidden",
  },
  {
    handle: "bottom",
    ariaLabel: "Resize panel from bottom edge",
    className:
      "absolute right-4 bottom-0 left-4 z-(--layer-panel-resize-handle) h-2 cursor-ns-resize bg-transparent focus:outline-none max-sm:hidden",
  },
];
...
export function ResizablePanel({
  ...
  return (
    <SideChatWidgetRoot
      className={cn(
        "sc-widget-panel z-(--layer-panel) flex flex-col overflow-hidden border border-border bg-card text-foreground shadow-(--shadow-panel)",
