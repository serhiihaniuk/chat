...
const capPromptInputFiles = ({
  ...
  const capped = typeof capacity === "number" ? files.slice(0, capacity) : [...files];
...
type PromptTextareaKeyboardEvent = Parameters<KeyboardEventHandler<HTMLTextAreaElement>>[0];

const isPlainEnterSubmit = (event: PromptTextareaKeyboardEvent, isComposing: boolean) =>
  event.key === "Enter" && !isComposing && !event.nativeEvent.isComposing && !event.shiftKey;
...
export type PromptInputProps = Omit<HTMLAttributes<HTMLFormElement>, "onSubmit" | "onError"> & {
  // e.g., "image/*" or leave undefined for all files
  accept?: string;
  multiple?: boolean;
  ...
  const inner = (
    <>
      ...
      <form className={cn("w-full", className)} onSubmit={handleSubmit} ref={formRef} {...props}>
        <InputGroup className="rounded-xl overflow-hidden">{children}</InputGroup>
      </form>
    </>
  );
