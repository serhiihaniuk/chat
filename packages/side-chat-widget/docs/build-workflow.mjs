...
const PREAMBLE = [
  ...
  "G3 Every utility is registered. Allowed colour utilities (bg-/text-/border-/ring-/fill-/stroke-): background, foreground, card(-foreground), popover(-foreground), primary(-foreground), secondary(-foreground), muted(-foreground), accent(-foreground), destructive (NO -foreground exists), success, sc-canvas, border, input, ring, sidebar(-foreground/-primary/-primary-foreground/-accent/-accent-foreground/-border/-ring), message-user(-foreground), chart-1..5. Radius: rounded-sm/md/lg/xl. Type (with paired leading): text-2xs text-xs text-sm text-base text-md text-lg text-xl. Named sizes: w-sidebar w-menu h-header size-control size-touch max-w-measure-message max-w-measure-empty. Shadow: shadow-(--shadow-card) shadow-(--shadow-popover) shadow-(--shadow-panel). Ease: ease-out. Leading: leading-message. PLUS all standard Tailwind layout/spacing/flex utilities (flex, grid, items-*, justify-*, gap-*, p-*, px-*, py-*, m-*, w-*, h-*, size-8, min-w-0, truncate, rounded-full, absolute, relative, etc.) are fine. If a colour/size/radius/shadow you need is NOT in this list, use the component's hook class instead — do NOT invent a utility name.",
...
const COMPONENTS = [
...

  {
    ...
    partB: [
      ...
      'ITEMS are plain utilities + the highlighted: variant: className="cursor-pointer select-none rounded-md px-2.5 py-2 highlighted:bg-accent". Separator: className="my-1.5 h-px bg-border". GroupLabel: className="px-2.5 pt-1.5 pb-1 text-2xs font-bold uppercase tracking-wider text-muted-foreground".',
...

  {
    ...
    partB: [
      ...
      "TRACK = sc-seg hook class. ITEM classes: flex-1 flex items-center justify-center gap-1.5 rounded-sm px-1.5 py-1.5 text-xs font-medium text-muted-foreground cursor-pointer + pressed:bg-background pressed:text-foreground pressed:shadow-(--shadow-card). Active needs fill+shadow, not colour alone.",
...

  {
    ...
    partB: [
      ...
      'TRIGGER classes: a sc-icon-btn variant is fine, e.g. className="sc-icon-btn w-full justify-between px-3 rounded-xl border border-input". ITEM: flex cursor-pointer select-none items-center gap-2.5 rounded-md px-2.5 py-2 highlighted:bg-accent. ItemIndicator: ml-auto opacity-0 selected:opacity-100 text-primary.',
...

  {
    ...
    partB: [
      ...
      "INPUT: w-full bg-transparent text-sm outline-none placeholder:text-muted-foreground. ITEM: flex cursor-pointer select-none items-center gap-2.5 rounded-md px-2.5 py-2 highlighted:bg-accent, with a leading sc-media glyph + a min-w-0 truncate title/desc column + ItemIndicator (ml-auto opacity-0 selected:opacity-100 text-primary).",
...

  {
    ...
    partB: [
      ...
      "PORTAL container=container (G5). Popup classes (Tooltip has no dedicated palette — inherit menu colours): rounded-md bg-popover px-2 py-1 text-xs text-popover-foreground border border-border shadow-(--shadow-popover) starting:opacity-0 ending:opacity-0.",
...

  {
    ...
    partB: [
      ...
      "PORTAL container=container (G5). Items use cursor-pointer select-none plus highlighted:; checkbox checked:. One popover open at a time (Base UI).",
...

  {
    ...
    partB: [
      ...
      "Tabs.Root stays mounted in BOTH layouts; WIDE renders Tabs.List as a left rail of two-line row items (title + short description) using --settings-item-* tokens, where --settings-item-active-bg/hover/title/desc/indicator alias the conversation row tokens. NARROW swaps the List for a Select bound to the SAME value. Tabs.Panel per group identical in both. ONE GROUPS array drives both navigators -> content never forks.",
      'GROUPS: Theme (ThemePreviewCard selector cards — two columns in the wide body, each card has a preview band scoped with data-sidechat-theme-preview="graphite|sapphire|sage|ocean" and consumes --sc-canvas/--card/--border/--muted/--primary in isolation) and General (one Send with Ctrl+Enter Switch row). The settings body scrolls via ScrollArea.',
