...
| `protocol/protocol-event-stream.ts`                    | Post-start protocol event stream (`createStartedProtocolStream` over refs from `createProtocolStreamRefs`): emits `sidechat.started`, maps runtime events, and gates emission through the state machine. Finalization-free so the runner owns terminal persistence. |
| `protocol/run-turn-generation.ts`                      | Server-owned generation: drains the post-start stream to the durable event log under an owner lease and owns finalization through `Effect.onExit`. The service runner forks this off the request.                                                                   |
...
- `createStartedProtocolStream` is the post-start stream the runner drains; it
  never finalizes, so terminal persistence stays in `run-turn-generation`.
