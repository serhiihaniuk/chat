...
| `run-turn-guards.ts`                                   | Pre-context guard execution and guard failure mapping.                                                                                                                                                                                                                     |
...
| `protocol/protocol-event-stream.ts`                    | Post-start protocol event stream (`createStartedProtocolStream` over refs from `createProtocolStreamRefs`): emits `sidechat.started`, maps runtime events, and gates emission through the state machine. Finalization-free so the runner owns durable status finalization. |
| `protocol/run-turn-generation.ts`                      | Server-owned generation: drains the post-start stream to the turn-event port under an owner lease and owns durable status finalization through `Effect.onExit`. The service runner forks this off the request.                                                             |
| `protocol/lease/turn-lease-heartbeat.ts`               | Owner-lease fencing inside the generation `onExit`: claims the lease, heartbeats it on the resolved interval, and self-interrupts the drain when fenced (a renew that matches no row) so a stale owner stops.                                                              |
...
| `protocol/finalization/finalize-turn-generation.ts`    | `Effect.onExit` finalizer for server-owned generation: durable status on a normal exit; on an abnormal exit one synthetic terminal classified from the exit cause + durable cancel intent (user cancel vs shutdown/fence vs defect).                                       |
| `stream-chat-observability.ts`                         | Effect wrapper around stream lifecycle observation.                                                                                                                                                                                                                        |
...
- `createStartedProtocolStream` is the post-start stream the runner drains; it
  never finalizes, so durable turn-status finalization stays in
  `run-turn-generation`.
- Apps provide one concrete `StreamChatPorts` object to the pre-start and runner
  entrypoints; core has no Effect Layer or service registry.
- Stream-to-`ReadableStream`/`AsyncIterable` conversion belongs at the HTTP/SSE
  response boundary. Same-instance resume reads the retained event suffix from
  the service's per-instance registry.
