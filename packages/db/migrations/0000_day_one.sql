...
--> statement-breakpoint
CREATE TABLE "sidechat"."client_tool_dispatches" (
	"client_tool_dispatch_id" text PRIMARY KEY NOT NULL,
	...
	"completed_at" timestamp with time zone,
	"late_result_at" timestamp with time zone,
	CONSTRAINT "client_tool_dispatches_state_check" CHECK (state in ('dispatched', 'settled', 'failed', 'timed_out', 'late', 'aborted'))
);
--> statement-breakpoint
CREATE TABLE "sidechat"."conversation_title_runs" (
...
--> statement-breakpoint
CREATE TABLE "sidechat"."tool_approvals" (
	...
	CONSTRAINT "tool_approvals_state_check" CHECK (state in ('requested', 'approved', 'denied', 'expired'))
);
--> statement-breakpoint
...
CREATE UNIQUE INDEX "usage_records_turn_step_uq" ON "sidechat"."usage_records" USING btree ("assistant_turn_id","runtime_step_index");--> statement-breakpoint
