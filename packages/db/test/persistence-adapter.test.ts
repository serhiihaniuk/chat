...
describe('sidechat persistence adapter', () => {
  it('adapts stored procedures to conversation and usage ports', async () => {
    ...
    const persistence = createSideChatPersistence({
      async query(text, params) {
        ...
        if (text.includes('sidechat_create_or_get_conversation')) return { rows: [{ conversation_id: 'conv-db-1' }] }
        return { rows: [] }
      }
    })

    await expect(persistence.conversations.createOrGet({ workspaceId: 'demo-workspace', userId: 'demo-user', conversationId: 'demo-conversation-001' })).resolves.toBe('conv-db-1')
    await persistence.conversations.appendUserMessage('conv-db-1', 'client-msg-001', 'hello')
    await persistence.conversations.appendAssistantMessage('conv-db-1', 'assistant-msg-001', 'hi', { provider: 'openai', id: 'gpt-4.1-mini' })
  ...
  it('uses the provided close hook for runtime pool cleanup', async () => {
    ...
    const persistence = createSideChatPersistence({ query: async () => ({ rows: [] }) }, close)
