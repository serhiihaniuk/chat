...
Source of truth for: the shared, browser-safe wire vocabulary imported by both the service scrub filter and the widget, including the separate turn-activity stream names.
...
This package is deliberately tiny: browser-safe contract types, runtime validators,
and the error vocabulary table. It has no dependencies and is safe to import in a
browser bundle.
...
- `SideChatDataParts`: the `data-*` extension point. Empty at baseline — turn state derives from native parts and message metadata (ADR 0007). Adding a part requires a schema, a named consumer, and a privacy review.
...
- `SIDE_CHAT_CLIENT_TOOL_CAPABILITY`: the shared header name and fixed entropy/
  encoding shape for originating-tab client-tool authority. The raw header value
  is private HTTP input and is never part of the public UI message stream.
- `TURN_ACTIVITY_EVENT_TYPE` / `TURN_ACTIVITY_SYNC_EVENT_TYPE`: the shared names
  for the separate advisory activity SSE stream consumed by the widget.
