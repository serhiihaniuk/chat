...
for (const packageJsonPath of listWorkspacePackageJsons(root)) {
  ...
  if (packageJson.version !== "0.0.0")
    errors.push(`${packageJsonPath}: version must be 0.0.0`);
  if (packageJson.private !== true)
    errors.push(
      `${packageJsonPath}: private must be true before publishing ADRs`,
    );
  if (packageJson.type !== "module")
    errors.push(`${packageJsonPath}: type must be module`);
  if (!packageJson.exports?.["."])
    errors.push(`${packageJsonPath}: public entrypoint export is required`);
  if (!packageJson.types)
    errors.push(`${packageJsonPath}: types path is required`);
