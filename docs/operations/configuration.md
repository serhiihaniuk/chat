...
The AI SDK 7 replacement wing follows the same readable-declaration rule in [`apps/side-chat-service/sidechat.config.ts`](../../apps/side-chat-service/sidechat.config.ts), with standalone `azure` and testing-only `fake` variants. Its smaller Step 04 surface is `models`, `auth`, `timeouts`, `agent`, `capacity`, `keepalive`, `telemetry`, and `workflow`. `SERVICE_ENV_KEYS` in `src/config/declaration/side-chat-config.ts` is the only vocabulary for its environment names; property names and values are uppercase, and config files never inline environment strings. Production selects only OpenAI or Azure. The scripted model is accepted only by testing composition. Telemetry mode is `off`, `console`, or `otlp`; OTLP requires `endpoint` and `serviceName`, commonly referenced through `SERVICE_ENV_KEYS.SIDECHAT_OTLP_ENDPOINT` and `SERVICE_ENV_KEYS.SIDECHAT_OTEL_SERVICE_NAME`. Its exporter packages are optional and loaded only for that mode.
