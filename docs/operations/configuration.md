...
| `resumability`       | Lease and heartbeat timers, the per-process `instanceId`, and the crash-recovery sweep cadence (`reaperInterval`, `reaperBatchLimit` — ADR 0008). Known gap: the `turnEventRetention` and `prunerInterval`/`prunerBatchLimit` keys configure nothing since the durable-log removal; deletion is `plan/10`. | `:189` |
