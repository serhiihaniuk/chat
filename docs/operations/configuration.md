...
The AI SDK 7 replacement wing follows the same readable-declaration rule in [`apps/side-chat-service/sidechat.config.ts`](../../apps/side-chat-service/sidechat.config.ts), with standalone `azure` and testing-only `fake` variants. Its smaller surface is `models`, `auth`, `timeouts`, `agent`, `capacity`, `keepalive`, `telemetry`, and `workflow`; `models.titleModelId` selects the isolated conversation-title job, `timeouts.titleMs` bounds that job, and `agent.instructions` is the explicit system instruction used by each turn WorkflowAgent. `SERVICE_ENV_KEYS` owns service-wide environment names, while the OpenAI and Azure provider catalogs own their credentials, transport inputs, model ids, and supported reasoning values. Catalog properties are uppercase, and config files never inline closed values or environment names. Production selects only OpenAI or Azure. The scripted model is accepted only by testing composition. Telemetry mode is `off`, `console`, or `otlp`; OTLP requires `endpoint` and `serviceName`, commonly referenced through `SERVICE_ENV_KEYS.SIDECHAT_OTLP_ENDPOINT` and `SERVICE_ENV_KEYS.SIDECHAT_OTEL_SERVICE_NAME`. Its exporter packages are optional and loaded only for that mode.
