...
The composition root assembles every port into one `StreamChatPorts` object at [create-stream-chat-ports.ts:47](../../apps/partner-ai-service/src/composition/factories/create-stream-chat-ports.ts). Routes receive that object; they never build adapters.
