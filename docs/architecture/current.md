...
| `apps/side-chat-api/src/application/stream-chat.ts`, `stream-chat/`, and `stream-chat-request-schema.ts` | The Effect entrypoint delegates request decoding to shared protocol validation, then the main use case checks model/auth/rate/billing, loads context/history, appends messages, streams model chunks, emits sidechat events, records usage, and enriches citations/attachments. Focused helpers under `stream-chat/` own event creation, usage enrichment, metadata, and surface-context resolution. | A clearer application workflow, still centered on ports, with Effect services/layers added only where they clarify dependencies and lifecycle. | This is the best place to teach the application core: it knows the conversation, not Hono, React, Postgres, or provider SDK details. | Async generator complexity can hide lifecycle and error rules. |
...
| `apps/side-chat-api/src/adapters/workbench/workbench-tools-adapter.ts` and `workbench-tools/` | Workbench data lookup now lives behind `WorkbenchToolsPort`, while fallback data, citation shaping, and current surface/table view logic are split under `workbench-tools/`. | Keep Workbench tool policy outside Hono and keep the adapter as composition, not business logic storage. | Assistant tools need approved Workbench data, but Hono should not own dashboard query policy. | Surface filtering must stay aligned with the host grid semantics. |
