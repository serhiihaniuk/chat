...
| Persistence         | Schema, repository contracts, Drizzle/Postgres adapters, in-memory repositories, the dedicated `LISTEN/NOTIFY` connection and notification source. | `shared`, Drizzle/Postgres, Effect (Stream for the notification source), package-local test helpers. | Hono, React, widget, runtime internals, core use cases.                      | Importing protocol DTOs only to get generic JSON shapes.                        |
