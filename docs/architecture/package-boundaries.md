...
| Runtime           | Prepared assistant turn execution, executor registry, runtime tools, provider adapter, RuntimeEvents. | `shared`, Effect, AI SDK/provider packages inside adapter folders.       | Product authorization, DB rows, Hono, React, protocol DTOs.       | Letting runtime decide product policy or expose provider stream parts.          |
| Core workflow     | Authorization, turn policy, context, ports, lifecycle, protocol mapping.                              | `chat-protocol`, `shared`, Effect, runtime contract types through ports. | Hono, Drizzle/Postgres, provider SDKs, React, widget state.       | Pulling service adapters or DB implementation into core to simplify a use case. |
| Service adapter   | HTTP/Hono, auth/config, concrete ports, app adapters, service composition, SSE transport.             | Core, runtime providers, DB adapters, protocol, Hono.                    | Widget internals, copied UI, product lifecycle ownership.         | Putting tool, RAG, memory, or guard logic directly in routes.                   |
...
| Persistence       | Schema, repository contracts, Drizzle/Postgres adapters, memory repositories.                         | `shared`, Drizzle/Postgres, package-local test helpers.                  | Hono, React, widget, runtime internals, core use cases.           | Importing protocol DTOs only to get generic JSON shapes.                        |
