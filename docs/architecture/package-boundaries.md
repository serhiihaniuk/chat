...
| `apps/side-chat-service` (v7 greenfield wing)   | AI SDK, Workflow, Hono, Effect, `chat-protocol`, `db`, `stream-profile`, and its package-private hexagonal layers | application importing outer layers; adapter coupling; Workflow physics hidden outside `workflows`; production importing doubles | `check-side-chat-service-architecture`, `check-dependency-policy`         |
