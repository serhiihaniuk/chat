...
| `apps/side-chat-service` (v7 greenfield wing)   | AI SDK, Workflow, Hono, Zod, and its package-private hexagonal layers                                          | inward layers importing adapters/bootstrap; adapters importing bootstrap                                          | `check-side-chat-service-architecture`, `check-dependency-policy`         |
...
- `process.env` -> only the active service's configuration adapter (`apps/partner-ai-service/src/config/` or `apps/side-chat-service/src/adapters/configuration/`) and `*.test.ts`. Everything else takes config through ports.
...
| _`check-side-chat-service-architecture.mjs`_ | The v7 service dependency direction is `bootstrap -> adapters -> application -> ports`; ports remain dependency-free, application stays framework-free, and adapters cannot import bootstrap.                                                                                                                                                                           |
