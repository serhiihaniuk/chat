...
|     5 | turn guards        | Runs only guard ids selected by the profile safety policy.                                        | Pre-start rejection.                               | Allow, warning, or block decision.         |
...
|    11 | runtime stream     | Executes AgentRuntimeRequest through the profile-selected AgentExecutor.                          | Runtime failure maps to post-start terminal error. | RuntimeEvents.                             |
