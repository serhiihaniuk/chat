...
| Term          | Meaning                                                                                                                                   | Owner                          |
| ------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------ |
| Tenant        | Adopter-owned top-level isolation domain. Side Chat represents it through a globally unique, tenant-qualified `workspaceId`, not a field. | Adopter auth integration       |
| Workspace     | Durable authorization scope inside a tenant. Its public `workspaceId` must never be reused across tenants.                                | Adopter auth integration       |
| Subject       | Authenticated principal inside a tenant-qualified workspace.                                                                              | Adopter auth integration       |
| `AuthContext` | Request-scoped workspace/subject identity plus issue time. Credentials and mutable claims are intentionally excluded.                     | Server framework and HTTP edge |
...
| Durable actor reference | Secret-free tenant-qualified workspace and subject identity carried through Workflow so tools can authorize the current actor. | Server framework and Workflow input          |
