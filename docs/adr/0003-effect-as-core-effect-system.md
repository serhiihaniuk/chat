...
| **Dependency injection without a framework.** Effects are lazy descriptions, so capabilities arrive as plain injected port objects; tests swap in fakes with zero mocking libraries.                                  | `composePartnerAiService` wires real adapters; `src/testing/` fakes drive the whole spine in unit tests. Deliberately plain ports objects — not Effect Layers (`plan/24` deletes the unused Layer path). | Module-level singletons or a DI container dependency; mock frameworks patching imports.                                   |
