...
| **Dependency injection without a framework.** Effects are lazy descriptions, so capabilities arrive as plain injected port objects; tests swap in fakes with zero mocking libraries.                                  | `composePartnerAiService` wires real adapters; `src/testing/` fakes drive the whole spine in unit tests. Ports deliberately remain plain objects, not Effect Layers. | Module-level singletons or a DI container dependency; mock frameworks patching imports.                                   |
...
- Every seam an adopter touches is Effect-optional or Effect-trivial: tools get
  the promise-based `createRuntimeToolFromPromise` factory, the diagnostic logger is plain sync
