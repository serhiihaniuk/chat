...
COPY .npmrc package.json package-lock.json tsconfig.json tsconfig.base.json tsconfig.check.json ./
...
COPY packages/partner-ai-core packages/partner-ai-core
COPY packages/chat-protocol packages/chat-protocol
COPY packages/db packages/db

RUN npm run build --workspace @side-chat/partner-ai-service \
  && npm prune --omit=dev
