import type { ModelSelection, SidechatRequest, SidechatStreamEvent, TokenUsage } from '@side-chat/shared-protocol'

export type ModelChunk = { kind: 'delta'; text: string } | { kind: 'done'; finishReason: string; usage: TokenUsage }

export interface ModelPort {
  stream(request: SidechatRequest, signal?: AbortSignal): AsyncIterable<ModelChunk>
}
...
export interface ConversationRepository {
  ...
  readSeededHistory(workspaceId: string, conversationId: string): Promise<Array<{ id: string; role: 'user' | 'assistant' | 'system'; content: string }>>
}
