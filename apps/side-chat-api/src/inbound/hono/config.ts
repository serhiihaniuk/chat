...
const sidechatEnvSchema = z.object({
  ...
  SIDE_CHAT_RATE_LIMITING_ENABLED: z.preprocess(
    (value) => {
  ...
  SIDE_CHAT_BILLING_ENABLED: z.preprocess(
    (value) => {
      if (value === undefined) return true
      return parseBooleanLike(String(value))
    },
...
export const parseSideChatEnv = (env: NodeJS.ProcessEnv = process.env): SideChatApiEnv => {
