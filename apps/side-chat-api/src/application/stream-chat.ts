import { streamRequestSchema, type SideChatStreamEvent, type StreamRequest } from '@side-chat/shared-protocol'
...
export async function* streamChat(deps: StreamChatDeps, input: StreamChatInput): AsyncIterable<SideChatStreamEvent> {
  const request = streamRequestSchema.parse(input.body) satisfies StreamRequest
  ...
  for await (const chunk of deps.model.stream(request, input.signal)) {
    if (chunk.kind === 'delta') {
      ...
      const event: SideChatStreamEvent = { type: 'sidechat.delta', delta: chunk.text }
      ...
      try { await deps.usage.record({ requestId: input.requestId, conversationId, messageId: assistantMessageId, model: request.model, usage: chunk.usage }) } catch { throw new UsageCaptureFailed() }
      const completed: SideChatStreamEvent = { type: 'sidechat.completed', finishReason: chunk.finishReason, model: request.model, usage: chunk.usage }
