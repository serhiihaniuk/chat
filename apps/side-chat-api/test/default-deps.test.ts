...
describe('default dependency wiring from environment', () => {
  ...
  it('applies workspace allowlist authorization', async () => {
    ...
    await expect(deps.auth.authorize('demo-workspace', 'u-1')).resolves.toBe(true)
  ...
  it('applies workspace blocklist authorization', async () => {
    ...
    await expect(deps.auth.authorize('blocked-workspace', 'u-1')).resolves.toBe(false)
    await expect(deps.auth.authorize('ok-workspace', 'u-1')).resolves.toBe(true)
  })
