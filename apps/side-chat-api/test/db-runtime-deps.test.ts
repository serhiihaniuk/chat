...
describe("default deps", () => {
  ...
  it("uses in-memory conversation repository when DATABASE_URL is not set", async () => {
    ...
    const id = await deps.conversations.createOrGet({
  ...
  it("uses stored-procedure DB adapter when DATABASE_URL is set", async () => {
    process.env.DATABASE_URL =
      "postgres://sidechat_app:sidechat_app@postgres:5432/sidechat";
    const deps = createDefaultDeps();
    const id = await deps.conversations.createOrGet({
      workspaceId: "demo-workspace",
      userId: "demo-user",
    ...
    expect(vi.mocked(createPostgresSideChatPersistence)).toHaveBeenCalledWith(
      process.env.DATABASE_URL,
    );
    expect(persistence.createOrGet).toHaveBeenCalledWith({
      workspaceId: "demo-workspace",
      userId: "demo-user",
      conversationId: "demo-conversation-001",
    });
