    ...
    <main className="demo-shell">
      <section>
        ...
        <div className="demo-card-grid" aria-label="Demo state checklist">
          <article><strong>Seeded history</strong><span>Opens demo-conversation-001 on launch.</span></article>
          <article><strong>Streaming markdown</strong><span>Try headings, lists, links, and code prompts.</span></article>
          <article><strong>Error / retry</strong><span>Send a prompt containing “fail” to exercise stream errors.</span></article>
        </div>
        <aside className="demo-events" aria-label="Widget callback events">
          <h2>Widget callbacks</h2>
          {events.length === 0 ? <p>No widget events yet.</p> : <ul>{events.map((event) => <li key={event}>{event}</li>)}</ul>}
        </aside>
      </section>
      <SideChatWidget
